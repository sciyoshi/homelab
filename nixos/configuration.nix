let
  hostConfig = builtins.fromJSON (builtins.readFile ./host-config.json);
in { pkgs, config, ... }: {
  imports = [
    (./hardware + "/${hostConfig.profile}.nix")
    "${builtins.fetchTarball "https://github.com/Mic92/sops-nix/archive/master.tar.gz"}/modules/sops"
  ];

  boot.cleanTmpDir = true;

  boot.loader.grub.device = hostConfig.bootDevice;
  fileSystems."/" = { device = hostConfig.rootDevice; fsType = "ext4"; };
